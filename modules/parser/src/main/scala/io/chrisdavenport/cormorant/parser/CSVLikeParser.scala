...
/** This CSVParser tries to stay fairly close to the initial specification
  ...
  *
  * Deviations from the specification, here I have chosen to use a permissive
  * CRLF that will accept a CRLF, LF, or CR. Note that the CR is not directly in
  * the initial spec, but in rare cases csvs can have this delimiter
  *
  ...
  *
  * field_name,field_name,field_name CRLF aaa,bbb,ccc CRLF zzz,yyy,xxx CRLF 4.
  ...
  * ignored. The last field in the record must not be followed by a comma. For
  * example:
  *
  * aaa,bbb,ccc
  *
  *   5. Each field may or may not be enclosed in double quotes (however some
  *      programs, such as Microsoft Excel, do not use double quotes at all). If
  *      fields are not enclosed with double quotes, then double quotes may not
  *      appear inside the fields. For example:
  *
  * "aaa","bbb","ccc" CRLF zzz,yyy,xxx
  *
...
abstract class CSVLikeParser(val separator: Char) {
  ...
  val TEXTDATA: Parser[Char] =
    noneOf(dquoteS + separatorS + crS + lfS).named("TEXTDATA")

  // escaped = DQUOTE *(TEXTDATA / COMMA / CR / LF / 2DQUOTE) DQUOTE
  val escaped: Parser[CSV.Field] =
    (DQUOTE ~> many(
      TEXTDATA | SEPARATOR | CR | LF | TWO_DQUOTE.map(_ => dquote)
    )
      .map(_.mkString)
